...
module Api
  class NotesControllerTest < ActionDispatch::IntegrationTest
    ...
    def test_comment_success
      ...
      assert_difference "NoteComment.count", 1 do
        assert_no_difference "ActionMailer::Base.deliveries.size" do
          perform_enqueued_jobs do
            post comment_api_note_path(:id => note, :text => "This is an additional comment", :format => "json"), :headers => auth_header
          end
        end
      end
      assert_response :success
      ...
      get api_note_path(:id => note, :format => "json")
      assert_response :success
      js = ActiveSupport::JSON.decode(@response.body)
      assert_not_nil js
      assert_equal "Feature", js["type"]
    ...
    def test_comment_fail
      ...
      assert_no_difference "NoteComment.count" do
        post comment_api_note_path(:id => note)
      ...
      assert_no_difference "NoteComment.count" do
        post comment_api_note_path(:id => note), :headers => auth_header
      end
      assert_response :bad_request

      assert_no_difference "NoteComment.count" do
        post comment_api_note_path(:id => note, :text => ""), :headers => auth_header
      end
      assert_response :bad_request
      ...
      assert_no_difference "NoteComment.count" do
        post comment_api_note_path(:id => hidden_note, :text => "This is an additional comment"), :headers => auth_header
      end
      assert_response :gone

      closed_note = create(:note, :status => "closed", :closed_at => Time.now.utc)

      assert_no_difference "NoteComment.count" do
        post comment_api_note_path(:id => closed_note, :text => "This is an additional comment"), :headers => auth_header
      end
      assert_response :conflict

      assert_no_difference "NoteComment.count" do
        post comment_api_note_path(:id => note, :text => "x\u0000y"), :headers => auth_header
      end
      assert_response :bad_request
    end
    ...
    def test_close_success
      ...
      post close_api_note_path(:id => note, :text => "This is a close comment", :format => "json")
      assert_response :unauthorized

      auth_header = basic_authorization_header user.email, "test"

      post close_api_note_path(:id => note, :text => "This is a close comment", :format => "json"), :headers => auth_header
      assert_response :success
      js = ActiveSupport::JSON.decode(@response.body)
      assert_not_nil js
      assert_equal "Feature", js["type"]
      ...
      get api_note_path(:id => note.id, :format => "json")
      assert_response :success
      js = ActiveSupport::JSON.decode(@response.body)
      assert_not_nil js
      assert_equal "Feature", js["type"]
    ...
    def test_close_fail
      ...
      post close_api_note_path(:id => hidden_note), :headers => auth_header
      assert_response :gone

      closed_note = create(:note, :status => "closed", :closed_at => Time.now.utc)

      post close_api_note_path(:id => closed_note), :headers => auth_header
      assert_response :conflict
    end
    ...
    def test_reopen_success
      ...
      post reopen_api_note_path(:id => closed_note, :text => "This is a reopen comment", :format => "json")
      assert_response :unauthorized

      auth_header = basic_authorization_header user.email, "test"

      post reopen_api_note_path(:id => closed_note, :text => "This is a reopen comment", :format => "json"), :headers => auth_header
      assert_response :success
      js = ActiveSupport::JSON.decode(@response.body)
      assert_not_nil js
      assert_equal "Feature", js["type"]
      assert_equal closed_note.id, js["properties"]["id"]
      assert_equal "open", js["properties"]["status"]
      assert_equal 2, js["properties"]["comments"].count
      ...
      get api_note_path(:id => closed_note, :format => "json")
      assert_response :success
      js = ActiveSupport::JSON.decode(@response.body)
      assert_not_nil js
      assert_equal "Feature", js["type"]
    ...
    def test_reopen_fail
      ...
      post reopen_api_note_path(:id => hidden_note)
      assert_response :unauthorized
      ...
      post reopen_api_note_path(:id => hidden_note), :headers => auth_header
      assert_response :gone

      note = create(:note)

      post reopen_api_note_path(:id => note), :headers => auth_header
      assert_response :conflict
    end
    ...
    def test_show_success
      ...
      get api_note_path(:id => note, :format => "xml")
      assert_response :success
      assert_equal "application/xml", @response.media_type
      assert_select "osm", :count => 1 do
        assert_select "note[lat='#{note.lat}'][lon='#{note.lon}']", :count => 1 do
          assert_select "id", note.id.to_s
          assert_select "url", api_note_url(note, :format => "xml")
          assert_select "comment_url", comment_api_note_url(note, :format => "xml")
          assert_select "close_url", close_api_note_url(note, :format => "xml")
          assert_select "date_created", note.created_at.to_s
          assert_select "status", note.status
      ...
      assert_select "rss", :count => 1 do
        assert_select "channel", :count => 1 do
          assert_select "item", :count => 1 do
            assert_select "link", note_url(note)
            assert_select "guid", api_note_url(note)
            assert_select "pubDate", note.created_at.to_fs(:rfc822)
            assert_select "geo|lat", note.lat.to_s
            assert_select "geo|long", note.lon.to_s
            assert_select "georss|point", "#{note.lon} #{note.lon}"
          end
        end
      end
      ...
      assert_equal "Feature", js["type"]
      assert_equal "Point", js["geometry"]["type"]
      assert_equal note.lat, js["geometry"]["coordinates"][0]
      assert_equal note.lon, js["geometry"]["coordinates"][1]
      assert_equal note.id, js["properties"]["id"]
      assert_equal api_note_url(note, :format => "json"), js["properties"]["url"]
      assert_equal comment_api_note_url(note, :format => "json"), js["properties"]["comment_url"]
      assert_equal close_api_note_url(note, :format => "json"), js["properties"]["close_url"]
      assert_equal note.created_at.to_s, js["properties"]["date_created"]
      assert_equal note.status, js["properties"]["status"]

      get api_note_path(:id => note, :format => "gpx")
      assert_response :success
      assert_equal "application/gpx+xml", @response.media_type
      assert_select "gpx", :count => 1 do
        assert_select "wpt[lat='#{note.lat}'][lon='#{note.lon}']", :count => 1 do
          assert_select "time", :count => 1
          assert_select "name", "Note: #{note.id}"
          assert_select "desc", :count => 1
          assert_select "link[href='http://www.example.com/note/#{note.id}']", :count => 1
          assert_select "extensions", :count => 1 do
            assert_select "id", note.id.to_s
            assert_select "url", api_note_url(note, :format => "gpx")
            assert_select "comment_url", comment_api_note_url(note, :format => "gpx")
            assert_select "close_url", close_api_note_url(note, :format => "gpx")
          end
        end
      end
    end
    ...
    def test_show_hidden_comment
      note_with_hidden_comment = create(:note, :body => "Valid comment for hidden note") do |note|
    ...
    def test_destroy_success
      ...
      delete api_note_path(:id => note, :text => "This is a hide comment", :format => "json")
      assert_response :unauthorized

      auth_header = basic_authorization_header user.email, "test"

      delete api_note_path(:id => note, :text => "This is a hide comment", :format => "json"), :headers => auth_header
      ...
      get api_note_path(:id => note, :format => "json"), :headers => auth_header
      assert_response :success
    ...
    def test_destroy_fail
      ...
      delete api_note_path(:id => hidden_note, :format => "json"), :headers => auth_header
      assert_response :gone
    end
